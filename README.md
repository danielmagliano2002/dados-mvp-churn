...
Este repositório contém o Produto Mínimo Viável (MVP) desenvolvido como requisito de avaliação para a disciplina de Programação Orientada a Objetos / Machine Learning, do Departamento de Engenharia de Produção.
...
4. **Auditoria de Modelo:** Identificação e correção de um viés metodológico (*Data Leakage*) causado por variáveis irrealistas no momento da predição (`CustomerID` e `Payment Delay`).
...
O modelo final otimizado (**Random Forest Classifier**) foi submetido à base de teste (dados inéditos) e obteve as seguintes métricas:
